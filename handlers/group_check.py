...
from loader import dp, db, groups, groups_list, week_lectures, notify_lectures, bot, FEEDBACK_CHAT, BACKUP_CHAT
...
async def new_chat_member(message: types.Message):
    for new_member in message.new_chat_members:
        if new_member.id == (await bot.me).id:
            if message.chat.id != FEEDBACK_CHAT and message.chat.id != BACKUP_CHAT:
                await message.answer(f"Вибачте, я не можу знаходитись у цьому чаті 😣")
